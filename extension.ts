...
function addMapsExtensionSwitch() {
  ...
  undergroundSwitchElement.addEventListener('click', () => {
    ...
    if (IS_UNDERGROUND_ACTIVE) {
      ...
      (<HTMLButtonElement>document.querySelector('[data-target=\'지하 지도\']')).click();
    } else {
      undergroundSwitchElement.classList.remove('on');
      (<HTMLButtonElement>document.querySelector('[data-target=\'지상 지도\']')).click();
    }
    setPinObjectRefresh();
  });
...
function getChestFilterValue(): string[] {
...
function isUndergroundEntrance(mapData: MapData): boolean {
  const entrancePinData: PinData[] = MAPS_PinLoad.filter((value: PinData) => value.name?.includes('지하 및 실내 구역 입구'));
  return entrancePinData.some((pinData: PinData) =>
    pinData.mapData && pinData.mapData.some((entranceMapData: MapData) => entranceMapData.id === mapData.id)
  );
}
...
removePin = ((originRemovePin) => {
  ...
  */
  function _makePinLoadProxy() {
    const target: PinData[] = MAPS_PinLoad.slice();
